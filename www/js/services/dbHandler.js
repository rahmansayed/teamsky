  ...
  .factory('dbHandler', function ($ionicPlatform, $cordovaSQLite, $q, $ionicLoading, $location, global) {
    ...
    function initDB() {
      ...
      if (window.cordova) {
        ...
        global.db = $cordovaSQLite.openDB({name: "teamSky1_6.db", location: 1, createFromLocation: 1});
        ...
        global.db = window.openDatabase("teamSky1_6.db", '1.0', 'Team Sky DB', 2 * 1024 * 1024);
      ...
      var query = [
        ...
        "CREATE TABLE IF NOT EXISTS listUser (listLocalId integer,contactLocalId integer,contactServerId text,privilage text,lastUpdateDate integer,lastUpdateBy text)",
          
        "CREATE TABLE IF NOT EXISTS contact (contactLocalId integer,contactName text,phoneNumber text UNIQUE,phoneType text,contactStatus text,lastUpdateDate integer,lastUpdateBy text,PRIMARY KEY(contactLocalId,phoneNumber))",
